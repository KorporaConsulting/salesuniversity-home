...
$(document).ready(function()
{
	...
	*/
	...
	{
		if($('.flash_slider').length)
		{
			var flashSlider = $('.flash_slider');
			flashSlider.owlCarousel(
			{
				center :true,
				loop:false,
				margin:30,
			...
			flashSlider.on('click', '.flash_fav', function (ev)
			{
			    $(ev.target).toggleClass('active');
			});

			if($('.flash_prev').length)
			{
				var prev = $('.flash_prev');
				prev.on('click', function()
				{
	...
	{
		if($('.all_slider').length)
		{
			var allSlider = $('.all_slider');
			...
			allSlider.on('click', '.all_fav', function (ev)
			{
			    $(ev.target).toggleClass('active');
			});
			...
			if($('.all_prev').length)
			{
				...
				prev.on('click', function()
				{
					allSlider.trigger('prev.owl.carousel');
				});
			}
			...
			if($('.all_next').length)
			{
				...
				next.on('click', function()
				{
					allSlider.trigger('next.owl.carousel');
				});
			}
		}
	}
